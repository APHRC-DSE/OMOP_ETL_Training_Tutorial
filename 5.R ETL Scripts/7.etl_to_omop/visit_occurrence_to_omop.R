...
visit_occurrence_cdm_table <- df_clean %>%
  ...
  dplyr::mutate(visit_occurrence_id = dplyr::row_number()
                ...
                , visit_source_value = "National Household Panel Survey Interview"
...
visit_occurrence_cdm_load <- 
  DBI::dbWriteTable(con
                    , name = Id(schema = cdm_schema_name, table = "visit_occurrence")
                    , value = visit_occurrence_cdm_table
                    , overwrite = TRUE
                    , row.names = FALSE
...
ALTER TABLE {cdm_schema_name}.visit_occurrence ADD CONSTRAINT xpk_visit_occurrence PRIMARY KEY (visit_occurrence_id);
                                                      ")
                 )
